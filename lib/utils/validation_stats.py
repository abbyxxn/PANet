...
class ValidationStats(object):
    ...
    def __init__(self, misc_args, log_period=20, number_val_iter=1, tensorboard_logger=None):
    ...
    def UpdateIterStats(self, model_out, inner_iter=None):
        ...
        if inner_iter is not None and self.number_val_iter > 1:
    ...
    def _UpdateIterStats_inner(self, model_out, inner_iter):
        ...
        assert inner_iter < self.number_val_iter
        ...
        for k, loss in model_out['losses'].items():
            ...
            if inner_iter == (self.number_val_iter - 1):
    ...
    def _mean_and_reset_inner_list(self, attr_name, key=None):
        ...
        if key:
            mean_val = sum(getattr(self, attr_name)[key]) / self.number_val_iter
            getattr(self, attr_name)[key] = []
        else:
            mean_val = sum(getattr(self, attr_name)) / self.number_val_iter
