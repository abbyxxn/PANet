...
from datasets.roidb import combined_roidb_for_training, combined_roidb_for_validation
...
def main():
    ...
    roidb_validation, ratio_list_validation, ratio_index_validation = combined_roidb_for_validation(
    ...
    #cfg.TRAIN.DATASET_SIZE = train_size
    ...
    # cfg.VALIDATION.DATASET_SIZE = validation_size    
    ...
    number_val_iter = validation_size // args.batch_size
    # cfg.VALIDATION.NUMBER_VAL_ITER = number_val_iter
             ...
             #               input_data_validation[key] = list(map(Variable, input_data_validation[key]))
                ...
                if (step + 1) % 60 == 0:
                    with torch.no_grad():
                        ...
                        for inner_iter_val in range(number_val_iter):
                            try:
                                input_data_validation = next(val_data_iterator)
                            except StopIteration:
                                val_data_iterator = iter(val_data_loader)
                            ...
                            validation_stats.UpdateIterStats(net_outputs_validation, inner_iter_val)
                        #maskRCNN.train()
